...
export function loadDatabaseConfig(overrides: Partial<DatabaseConfig> = {}): DatabaseConfig {
  const retry: RetryConfig = {
    maxRetries: Number(process.env['DATABASE_MAX_RETRIES'] || overrides.retry?.maxRetries || DEFAULT_RETRY.maxRetries),
    retryDelay: Number(process.env['DATABASE_RETRY_DELAY'] || overrides.retry?.retryDelay || DEFAULT_RETRY.retryDelay),
  };
  ...
  const pooling: PoolingConfig = {
    ...
    idleTimeoutMillis: Number(
      process.env['DATABASE_POOL_IDLE_TIMEOUT'] || overrides.pooling?.idleTimeoutMillis || DEFAULT_POOLING.idleTimeoutMillis
    ),
    ...
    maxUses: Number(process.env['DATABASE_POOL_MAX_USES'] || overrides.pooling?.maxUses || DEFAULT_POOLING.maxUses),
    reapIntervalMillis: Number(
      process.env['DATABASE_POOL_REAP_INTERVAL'] || overrides.pooling?.reapIntervalMillis || DEFAULT_POOLING.reapIntervalMillis
    ),
    allowExitOnIdle:
  ...
  return {
    ...
    mongo: {
      mongodbUrl: overrides.mongo?.mongodbUrl || process.env['MONGODB_URL'],
    ...
    logLevel: overrides.logLevel || (process.env['DATABASE_LOG_LEVEL'] as DatabaseConfig['logLevel']) || 'info',
    observability: {
      enableMetrics: overrides.observability?.enableMetrics ?? process.env['DATABASE_ENABLE_METRICS'] !== 'false',
      enableTracing: overrides.observability?.enableTracing ?? process.env['DATABASE_ENABLE_TRACING'] === 'true',
    ...
    shutdown: {
      gracefulTimeoutMs: overrides.shutdown?.gracefulTimeoutMs || Number(process.env['DATABASE_SHUTDOWN_TIMEOUT'] || 5000),
      waitForRequests: overrides.shutdown?.waitForRequests,
    },
    retry,
    pooling,
    unsafeAllowClearDatabase:
      overrides.unsafeAllowClearDatabase ?? process.env['ALLOW_CLEAR_DATABASE'] === 'true' || false,
    defaultAdminPassword: overrides.defaultAdminPassword || process.env['DEFAULT_ADMIN_PASSWORD'] || 'ChangeMe!123',
